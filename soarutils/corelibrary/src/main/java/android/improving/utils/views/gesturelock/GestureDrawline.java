...
public class GestureDrawline extends View {
    ...
    public GestureDrawline(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
    }

    public GestureDrawline(Context context, AttributeSet attrs, int defStype) {
        super(context, attrs, defStype);
        this.context = context;
    }


    public void init(List<GesturePoint> list, boolean isVerify,
        ...
        paint.setColor(getResources().getColor(R.color.color_50ffffff));// 设置默认连线颜色
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (currentPoint != null) {
                    currentPoint.setPointState(GestureContentView.POINT_STATE_SELECTED);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pointAt == null || currentPoint.equals(pointAt) || GestureContentView.POINT_STATE_SELECTED == pointAt.getPointState()) {
                    ...
                    pointAt.setPointState(GestureContentView.POINT_STATE_SELECTED);

                    // 判断是否中间点需要选中
                    GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
                    if (betweenPoint != null && GestureContentView.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
                        // 存在中间点并且没有被选中
                        ...
                        // 设置中间点选中
                        betweenPoint.setPointState(GestureContentView.POINT_STATE_SELECTED);
                        // 赋值当前的point;
                        currentPoint = pointAt;
                    } else {
    ...
    final class clearStateRunnable implements Runnable {
        public void run() {
            ...
            for (GesturePoint p : list) {
                p.setPointState(GestureContentView.POINT_STATE_NORMAL);
            }
            invalidate();
            isDrawEnable = true;
        }
    ...
    private void drawErrorPathTip() {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        paint.setColor(getResources().getColor(R.color.color_50f53952));// 设置默认线路颜色
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
            pair.first.setPointState(GestureContentView.POINT_STATE_WRONG);
            pair.second.setPointState(GestureContentView.POINT_STATE_WRONG);
            canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
                    pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
        }
        invalidate();
