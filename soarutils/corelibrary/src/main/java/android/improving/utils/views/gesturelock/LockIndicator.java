...
public class LockIndicator extends View {
    ...
    public LockIndicator(Context paramContext, AttributeSet paramAttributeSet) {
        ...
        patternNoraml = getResources().getDrawable(R.mipmap.gesture_pattern_node_normal);
        patternPressed = getResources().getDrawable(R.mipmap.gesture_pattern_node_selected);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < numRow; i++) {
            for (int j = 0; j < numColum; j++) {
                ...
                canvas.save();
                canvas.translate(i1, i2);//canvas.translate(10, 10);//把当前画布的原点移到(10,10),后面的操作都以(10,10)作为参照点，默认原点为(0,0)
