...
import android.improving.utils.views.cardsview.utilities.ValueInterpolator;
...
public class StackPageTransformer implements ViewPager.PageTransformer {
    ...
    private float calculateShiftForScale(float position, float scale, int dimen) {
        ...
        return mAboveStackSpace + ((mNumberOfStacked-1 + position) * mOverlap) + (dimen * 0.5f * (scale - 1));
    }
