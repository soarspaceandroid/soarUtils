...
public final class BitmapUtils {
    ...
    public static Bitmap getBitmapFromStream(InputStream is, int reqWidth,
                                             int reqHeight) {
        byte[] data = FileManager.input2byte(is);
    ...
    public static Bitmap blur(Context context, Bitmap sentBitmap, int radius) {
        ...
        if (radius < 0) {
            radius = 0;
            if (DEBUG) {
                LogPrint.wLogPrint(TAG, "radius must be 0 < r < 25 , forcing radius=0");
            }
        } else if (radius > 25) {
            radius = 25;
            if (DEBUG) {
                LogPrint.wLogPrint(TAG, "radius must be 0 < r < 25 , forcing radius=25");
            }
        }
